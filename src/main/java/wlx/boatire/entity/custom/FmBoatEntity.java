...
public class FmBoatEntity extends Entity implements VariantHolder<FmBoatEntity.FmType> {
    ...
    private static final TrackedData<Integer> DAMAGE_WOBBLE_SIDE = DataTracker.registerData(FmBoatEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Float> DAMAGE_WOBBLE_STRENGTH = DataTracker.registerData(FmBoatEntity.class, TrackedDataHandlerRegistry.FLOAT);
    private static final TrackedData<Integer> BOAT_TYPE = DataTracker.registerData(FmBoatEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Boolean> LEFT_PADDLE_MOVING = DataTracker.registerData(FmBoatEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    private static final TrackedData<Boolean> RIGHT_PADDLE_MOVING = DataTracker.registerData(FmBoatEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    private static final TrackedData<Integer> BUBBLE_WOBBLE_TICKS = DataTracker.registerData(FmBoatEntity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    private FmBoatEntity.Location location;
    private FmBoatEntity.Location lastLocation;
    ...
    public FmBoatEntity(World world, double x, double y, double z) {
        this(ModEntities.FM_BOAT_ENTITY, world);
    ...
    protected void initDataTracker() {
        ...
        this.dataTracker.startTracking(BOAT_TYPE, FmBoatEntity.FmType.OAK.ordinal());
    ...
    public void pushAwayFrom(Entity entity) {
        if (entity instanceof FmBoatEntity) {
    ...
    public void tick() {
        ...
        if (this.location != FmBoatEntity.Location.UNDER_WATER && this.location != FmBoatEntity.Location.UNDER_FLOWING_WATER) {
        ...
        if (this.getTireDur()<=0){
            ...
            this.TIRE_BROKEN=true;
        }
        else if (this.TIRE_BROKEN){this.TIRE_BROKEN=false;}

        if (this.getTireDur()>0&&this.getTireDur()<this.getMaxTireDur(this.getLoadedTire())*0.2F){
            this.TIRE_LOW = true;
        }
        else if(this.TIRE_LOW){this.TIRE_LOW=false;}
    ...
    private void updatePositionAndRotation() {
        ...
        if (this.field_7708 > 0) {
            double d = this.getX() + (this.x - this.getX()) / this.field_7708;
            double e = this.getY() + (this.y - this.getY()) / this.field_7708;
            double f = this.getZ() + (this.z - this.getZ()) / this.field_7708;
            double g = MathHelper.wrapDegrees(this.boatYaw - this.getYaw());
            this.setYaw(this.getYaw() + (float)g / this.field_7708);
            this.setPitch(this.getPitch() + (float)(this.boatPitch - this.getPitch()) / this.field_7708);
            this.field_7708--;
    ...
    private FmBoatEntity.Location checkLocation() {
        FmBoatEntity.Location location = this.getUnderWaterLocation();
        if (location != null) {
            this.waterLevel = this.getBoundingBox().maxY;
            return location;
        } else if (this.checkBoatInWater()) {
            return FmBoatEntity.Location.IN_WATER;
        } else {
            float f = this.getNearbySlipperiness();
            if (f > 0.0F) {
                this.nearbySlipperiness = f;
                return FmBoatEntity.Location.ON_LAND;
            } else {
                return FmBoatEntity.Location.IN_AIR;
            }
        }
    }
                    ...
                    for (int s = k; s < l; s++) {
                        if (r <= 0 || s != k && s != l - 1) {
                            ...
                            if (!(blockState.getBlock() instanceof LilyPadBlock)
                                    && VoxelShapes.matchesAnywhere(blockState.getCollisionShape(this.getWorld(), mutable).offset(p, s, q), voxelShape, BooleanBiFunction.AND)) {
    ...
    @Nullable
    private FmBoatEntity.Location getUnderWaterLocation() {
        Box box = this.getBoundingBox();
                ...
                for (int q = m; q < n; q++) {
                    ...
                    if (fluidState.isIn(FluidTags.WATER) && d < mutable.getY() + fluidState.getHeight(this.getWorld(), mutable)) {
                        if (!fluidState.isStill()) {
                            return FmBoatEntity.Location.UNDER_FLOWING_WATER;
                        }
        ...
        return bl ? FmBoatEntity.Location.UNDER_WATER : null;
    }
    ...
    private void updateVelocity() {
        ...
        if (this.lastLocation == FmBoatEntity.Location.IN_AIR && this.location != FmBoatEntity.Location.IN_AIR && this.location != FmBoatEntity.Location.ON_LAND) {
            this.waterLevel = this.getBodyY(1.0);
            this.setPosition(this.getX(), this.getWaterHeightBelow() - this.getHeight() + 0.101, this.getZ());
            this.setVelocity(this.getVelocity().multiply(1.0, 0.0, 1.0));
            this.fallVelocity = 0.0;
            this.location = FmBoatEntity.Location.IN_WATER;
        } else {
            if (this.location == FmBoatEntity.Location.IN_WATER) {
                f = (this.waterLevel - this.getY()) / this.getHeight();
                this.velocityDecay = 0.9F;
            } else if (this.location == FmBoatEntity.Location.UNDER_FLOWING_WATER) {
                e = -7.0E-4;
                this.velocityDecay = 0.9F;
            } else if (this.location == FmBoatEntity.Location.UNDER_WATER) {
                f = 0.01F;
                this.velocityDecay = 0.45F;
            } else if (this.location == FmBoatEntity.Location.IN_AIR) {
                this.velocityDecay = 0.9F;
            } else if (this.location == FmBoatEntity.Location.ON_LAND) {
                this.velocityDecay = this.nearbySlipperiness;
                this.velocityDecay = this.getModifiedDecay();
                /*
            ...
            Vec3d vec3d = this.getVelocity();
            this.setVelocity(vec3d.x * this.velocityDecay, vec3d.y + e, vec3d.z * this.velocityDecay);
            this.yawVelocity = this.yawVelocity * this.getModifiedYawDecay();
            if (f > 0.0) {
    ...
    private void updatePaddles() {
        if (this.hasPassengers()) {
            ...
            if (this.pressingForward) {
                f += this.getModifiedThrust();
            }

            if (this.pressingBack) {
                f -= this.getModifiedBreak();
            }
    ...
    protected void updatePassengerPosition(Entity passenger, Entity.PositionUpdater positionUpdater) {
        if (this.hasPassenger(passenger)) {
            ...
            if (passenger instanceof AnimalEntity && this.getPassengerList().size() == this.getMaxPassengers()) {
                ...
                passenger.setBodyYaw(((AnimalEntity)passenger).bodyYaw + j);
                passenger.setHeadYaw(passenger.getHeadYaw() + j);
            }
        }
    }
    ...
    protected void readCustomDataFromNbt(NbtCompound nbt) {
        if (nbt.contains("FmType", NbtElement.STRING_TYPE)) {
            this.setVariant(FmBoatEntity.FmType.getType(nbt.getString("FmType")));
        }
        if (nbt.contains("LoadedTire")) {
            this.setLoadedTire(nbt.getInt("LoadedTire"),false);
        }
        ...
        if (!this.hasVehicle()) {
            if (onGround) {
                if (this.fallDistance > 3.0F) {
                    if (this.location != FmBoatEntity.Location.ON_LAND) {
                        this.onLanding();
                        return;
                    }
    ...
    public void setVariant(FmBoatEntity.FmType type) {
        this.dataTracker.set(BOAT_TYPE, type.ordinal());
    }

    public FmBoatEntity.FmType getVariant() {
        return FmBoatEntity.FmType.getType(this.dataTracker.get(BOAT_TYPE));
    }
    ...
    public boolean isSubmergedInWater() {
        return this.location == FmBoatEntity.Location.UNDER_WATER || this.location == FmBoatEntity.Location.UNDER_FLOWING_WATER;
    }
    ...
    public static enum FmType implements StringIdentifiable {
        ...
        public static FmBoatEntity.FmType getType(int type) {
            return (FmBoatEntity.FmType)BY_ID.apply(type);
        }

        public static FmBoatEntity.FmType getType(String name) {
            return (FmBoatEntity.FmType)CODEC.byId(name, OAK);
        }
    }
    ...
    public void setLoadedTire(int tire,boolean changeNew) {
        this.dataTracker.set(LOADED_TIRE, tire);
        if (changeNew) {this.dataTracker.set(TIRE_DUR, this.getMaxTireDur(tire)+0.0F);}
    }

    public float getTireDur(){return this.dataTracker.get(TIRE_DUR);}
